def kstest_applied(data1, data2):

# This script will apply the scipy stats package for KS Tests to your data structured as a matrix
...
# tendency as well as to define the proportion of bootstraps in which the ttest delivered a significant result.
    ...
    if a <= 10:
        x = data1[:, in_vars]
        y = data2[:, in_vars]
        coeff, p = stats.kstest(x, y)
    elif a > 10 and a <= 100:
        ii = 0
        coeff_dist = np.zeros(10)
        p_dist = np.zeros(10)
        while ii < 10:
            ...
            coeff_dist[ii], p_dist[ii] = stats.kstest(sub1, sub2)
            ii += 1
        coeff_avg = np.mean(coeff_dist)
        coeff_sem = np.std(coeff_dist)/np.sqrt(ii-1)
    ...
    # fig = plt.hist(p_dist)
    return coeff_avg, coeff_sem, p_avg, p_sem, sig_rate