...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\david\\Google Drive\\DJFolder\\MyWorks\\Research\\MyCoding\\David Code\\Python\\StochChoice.py',
...
for thisTrial in trials:
    ...
    r1 = (np.random.randint(100)/100)*30
    hitOffer1 = visual.Rect(
        win=win, name='hitOffer1',units='cm', 
        width=(10,r1)[0], height=(10,r1)[1],
        ori=0, pos=(-20, 0),
        lineWidth=1, lineColor=[1,1,1], lineColorSpace='rgb',
        fillColor=1.0, fillColorSpace='rgb',
        opacity=1, depth=-3.0, interpolate=True)
        
    r2 = (np.random.randint(100)/100)*30
    hitOffer2 = visual.Rect(
        win=win, name='hitOffer2',units='cm', 
        width=(10,r2)[0], height=(10,r2)[1],
        ori=0, pos=(20, 0),
        lineWidth=1, lineColor=[1,1,1], lineColorSpace='rgb',
        fillColor=1.0, fillColorSpace='rgb',
    ...
    # this could be where to randomize color
    sm = [255,255,0]
    med = [0,0,255]
    lar = [0,255,0]
    r3 = np.random.randint(1,4)
    if r3 == 1:
        hitOffer1.setFillColor(sm)
        magL = 1
    elif r3 == 2:
        hitOffer1.setFillColor(med)
        magL = 2
    elif r3 == 3:
        hitOffer1.setFillColor(lar)
        magL = 3
    r4 = np.random.randint(1,4)
    if r4 == 1:
        hitOffer2.setFillColor(sm)
        magR = 1
    elif r4 == 2:
        hitOffer2.setFillColor(med)
        magR = 2
    elif r4 == 3:
        hitOffer2.setFillColor(lar)
        magR = 3
    ...
    routineTimer.reset()
    # save the randomized offer info
    ...
    # update component parameters for each repeat
    # use offer params and choice to compute/record reward
    probL= r1/30
    probR = r2/30
    trials.addData('probL', r1)
    trials.addData('probR', r2)
    trials.addData('magL', magL)
    trials.addData('magR', magR)
    tupsize = (1,100)
    rlist = np.zeros(tupsize)
    
    # if click in left
    if mouse.clicked_name[-1] == hitOffer1:
        tupsize = (1,probL*100)
        rtemp = np.ones(tupsize)
        for iter in rtemp:
            rlist[0,iter] = rtemp[0,iter]
        pay = np.random.randint(np.size(rlist), size=1)
        payout = rlist[pay]
        if payout == 1:
            rew = magL
            if rew == 1:
                ...
                rewcolor = 
    # if click in right
    elif mouse.clicked_name[-1] == hitOffer2:
        tupsize = (1,probR*100)
        rtemp = np.ones(tupsize)
        for iter in rtemp:
            rlist[0,iter] = rtemp[0,iter]
